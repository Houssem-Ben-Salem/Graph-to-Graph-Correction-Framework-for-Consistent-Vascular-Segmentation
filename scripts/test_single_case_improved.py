#!/usr/bin/env python3
"""
Test the full pipeline on a single case with improved reconstruction
Uses the proper SDF-based reconstruction instead of naive sphere approach
"""
...
from src.models.reconstruction.volume_reconstructor import VolumeReconstructor, ReconstructionConfig
...
def apply_graph_correction(model, graph, device, patient_id):
    ...
    for i, node in enumerate(graph.nodes):
        if i < len(corrected_positions):
            ...
            if node_operations[i] == 0:  # Modify - keep the node
                node_mapping[i] = len(corrected_nodes)  # Map old index to new index
                corrected_nodes.append(corrected_node)
            else:  # Remove - skip this node
                removed_nodes += 1
        else:
            # Keep node that wasn't processed
            node_mapping[i] = len(corrected_nodes)
            corrected_nodes.append(node)
    
    # Update edges to use new node indices
    corrected_edges = []
    for edge in graph.edges:
        try:
            ...
            # Check if both nodes still exist after correction
            if old_source in node_mapping and old_target in node_mapping:
                # Create new edge with updated indices
                new_edge = edge.copy()
    ...
    corrected_graph = VascularGraph(
        ...
        edges=corrected_edges,  # Use corrected edges with proper indexing
...
def reconstruct_volume_with_sdf(corrected_graph, original_prediction, patient_id):
    """Reconstruct volume using proper SDF-based reconstruction"""
    
    logger.info(f"Starting SDF-based reconstruction for {patient_id}...")
    
    try:
        # Create reconstruction configuration
        config = ReconstructionConfig(
            target_resolution=original_prediction.shape,  # Match original size
            voxel_spacing=(1.0, 1.0, 1.0),  # Assume isotropic
            auto_compute_bounds=True,
            volume_padding=5.0,  # Smaller padding
            
            # Blending with original
            blend_with_original=True,
            original_blend_weight=0.2,  # Give more weight to reconstruction
            confidence_weighting=True,
            
            # Post-processing
            apply_morphological_closing=True,
            closing_kernel_size=2,  # Smaller kernel
            apply_smoothing=True,
            smoothing_sigma=0.8,
            
            # Quality control
            min_component_size=50,  # Smaller minimum size
        ...
        # Reconstruct volume
        reconstruction_result = reconstructor.reconstruct_from_graph(
        ...
        logger.info(f"SDF reconstruction completed:")
        logger.info(f"  Templates used: {reconstruction_result.templates_used}")
        logger.info(f"  Rendering time: {reconstruction_result.rendering_time:.2f}s")
        logger.info(f"  Total time: {reconstruction_result.total_time:.2f}s")
        logger.info(f"  Quality score: {reconstruction_result.reconstruction_quality:.3f}")
        logger.info(f"  Volume ratio: {reconstruction_result.volume_ratio:.3f}")
        logger.info(f"  Connectivity score: {reconstruction_result.connectivity_score:.3f}")
        
        if reconstruction_result.warnings:
            logger.info("Reconstruction warnings:")
            for warning in reconstruction_result.warnings:
                logger.info(f"  - {warning}")
        
        return reconstruction_result.reconstructed_mask, reconstruction_result
        ...
        # Fallback to improved simple reconstruction
        logger.info("Falling back to improved simple reconstruction...")
        return reconstruct_volume_simple_improved(corrected_graph, original_prediction.shape)


def reconstruct_volume_simple_improved(corrected_graph, target_shape):
    """Improved simple reconstruction as fallback"""
    
    logger.info("Using improved simple reconstruction...")
    
    # Initialize volume
    reconstructed_volume = np.zeros(target_shape, dtype=np.float32)
    
    # Create skeleton-based reconstruction
    for i, node in enumerate(corrected_graph.nodes):
        pos = node['position']
        radius = max(2.0, node.get('radius', 2.0))  # Larger default radius
        
        # Convert to voxel coordinates
        x, y, z = int(round(pos[0])), int(round(pos[1])), int(round(pos[2]))
        r = max(2, int(radius * 1.5))  # Scale up radius
        
        # Place soft sphere (Gaussian-like)
                ...
                for dz in range(-r*2, r*2+1):
                    ...
                    if distance <= r*2:
                        ...
                        if (0 <= nx < target_shape[0] and 
                            ...
                            reconstructed_volume[nx, ny, nz] = max(
                                reconstructed_volume[nx, ny, nz], weight
                            )
    
    # Connect nodes with thick edges
    for edge in corrected_graph.edges:
        try:
            ...
            if start_idx < len(corrected_graph.nodes) and end_idx < len(corrected_graph.nodes):
                start_pos = corrected_graph.nodes[start_idx]['position']
                end_pos = corrected_graph.nodes[end_idx]['position']
                
                # Average radius for edge
                start_radius = max(1.5, corrected_graph.nodes[start_idx].get('radius', 1.5))
                end_radius = max(1.5, corrected_graph.nodes[end_idx].get('radius', 1.5))
                
                # Draw thick line between nodes
                steps = max(5, int(np.linalg.norm(np.array(end_pos) - np.array(start_pos))))
                for i in range(steps + 1):
                    ...
                    # Draw thick sphere at each point
    ...
    # Convert to binary with better threshold
    binary_mask = (reconstructed_volume > 0.3).astype(np.uint8)
    
    # Apply morphological closing
    from scipy import ndimage
    kernel = np.ones((3, 3, 3))
    binary_mask = ndimage.binary_closing(binary_mask, structure=kernel).astype(np.uint8)
    
    logger.info(f"Simple reconstruction: {np.sum(binary_mask)} voxels")
...
def evaluate_single_case_improved(patient_id):
    """Run complete pipeline evaluation with improved reconstruction"""
    
    logger.info("="*80)
    logger.info(f"IMPROVED PIPELINE EVALUATION: {patient_id}")
    logger.info("="*80)
    ...
    try:
        ...
        # 4. Reconstruct volume using improved SDF method
        corrected_mask, reconstruction_result = reconstruct_volume_with_sdf(
        ...
        logger.info("\n" + "="*60)
        logger.info("IMPROVED RESULTS SUMMARY")
        logger.info("="*60)
        ...
        results = {
            ...
            'reconstruction_result': {
                'templates_used': reconstruction_result.templates_used if reconstruction_result else 0,
                'quality': reconstruction_result.reconstruction_quality if reconstruction_result else 0,
                'warnings': reconstruction_result.warnings if reconstruction_result else []
            },
            'metrics': metrics
        }
        
        results_path = Path(f'experiments/improved_single_case_results_{patient_id}.pkl')
        ...
        logger.error(f"Improved evaluation failed: {str(e)}")
        import traceback
        traceback.print_exc()
        return None
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test improved pipeline on single case')
    parser.add_argument('--patient-id', type=str, default='PA000005',
                       help='Patient ID to test (default: PA000005)')
    
    args = parser.parse_args()
    
    results = evaluate_single_case_improved(args.patient_id)
    
    if results:
        logger.info("Improved single case evaluation completed successfully!")
    else:
        logger.error("Improved single case evaluation failed!")
